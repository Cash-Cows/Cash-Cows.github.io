 ...
 */
abstract contract CashCowsClubAbstract is 
  Ownable,
  AccessControl, 
  ERC721C,
  IERC721Metadata 
{ 
  ...
  /**
   * @dev Allows the treasury to burn
   */
  function burn(uint256 tokenId) external {
    //Only the current treasury can burn a cow
    if(_msgSender() != address(treasury)) revert InvalidCall();
    _burn(tokenId, ERC721B.ownerOf(tokenId));
  }
