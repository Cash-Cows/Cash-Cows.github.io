window.addEventListener('web3sdk-ready', async _ => {
  ...
  window.addEventListener('web3sdk-connected', async _ => {
    ...
    Web3SDK.state.items = (await contract.game.read().items(row.characterId))
  ...
  window.addEventListener('modal-open-click', async (e) => {
    ...
    const prices = [ 
      ['dolla', contract.dolla.address, pricing[contract.dolla.address]?.price || '0'], 
  ...
  window.addEventListener('mint-click', async e => {
    ...
    const address = currency === 'eth' ? zero : contract.dolla.address
    ...
    await write(contract.game, method, args, () => {
