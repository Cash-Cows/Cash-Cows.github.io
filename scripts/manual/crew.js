...
// $ npx hardhat run scripts/manual/crew.js
...
async function main() {
  ...
  const template = 'INSERT INTO Crew (ID, CharacterID, Crew, ' 
    + 'Background, Hide, Mask, Eyes, Mouth, Nose, Belly, Earrings, '
    + 'Head, Outerwear, Neck, Level, Images, Rates, Loot) VALUES (%s);'

  const start = 4004
  const limit = start + 1000

  for (let i = start; i < limit && i < database.length; i++) {
    ...
    console.log(template.replace('%s', [
      ...
      `'${row.characterId}'`,
      `'${row.attributes.Crew}'`,
      `'${row.attributes.Background}'`,
      `'${row.attributes.Hide}'`,
      `'${row.attributes.Mask}'`,
      `'${row.attributes.Eyes}'`,
      `'${row.attributes.Mouth}'`,
      `'${row.attributes.Nose}'`,
      `'${row.attributes.Belly}'`,
      `'${row.attributes.Earrings}'`,
      `'${row.attributes.Head}'`,
      `'${row.attributes.Outwear}'`,
      `'${row.attributes.Neck}'`,
      `'${row.attributes.Level}'`,
      `'${JSON.stringify(row.images)}'`,
      `'${JSON.stringify(row.rates)}'`,
      `'${JSON.stringify(row.loot)}'`
    ].join(', ')))
    console.log('')
  }
}
