  ...
  for (const layer of layers) {
    ...
    for (const attribute in layer.attributes) {
      ...
      if (random < 0) {
        ...
        for (let i = 0; i < 150; i++) {
          layer.attributes[attribute].resources.push(
            await loadImage(path.join(
              paths.root, 
...
async function animate(canvas, attributes) {
  ...
  return {
    image: buffer,
    cid: await cid.of(buffer, { cidVersion: cid_version })
  }
}
...
function generateMetadata(config, edition, cid, attributes) {
  //generate the json
...
async function main() {
  ...
  for (let i = 0; i < images.length; i++) {
    ...
    const { image, cid } = await animate(canvas, attributes)
    //make metadata
    const metadata = generateMetadata(images[i].set, edition, cid, attributes)
    //save
    console.log('Saving', `${edition}`, images[i].set.config, cid)
    //save image
    fs.writeFileSync(
      path.join(paths.build, `image/${edition}.gif`), 
      image
    )
    //save json
    fs.writeFileSync(
      path.join(paths.build, `json/${edition}.json`), 
